...
function setElements(jsonObject) {
    ...
    /*for (var x = 0; x < movies.length; x++) {
        var film = document.querySelector(".film" [x]);

        film.addEventListener("click", function () {
            film[x].classList.toggle('liked');
            if (film[x].classList.contains('liked')) {
    ...
    var darkKnop = document.querySelector('nav li button');

    function darkmode() {
        body.classList.toggle("darkmode");
        darkKnop.classList.toggle("darkmode");
    ...
    window.onkeyup = function pressE(e) {
        if (e.keyCode == 68) {
            body.classList.toggle("darkmode");
            darkKnop.classList.toggle("darkmode");
            articles.classList.toggle("darkmode2");
